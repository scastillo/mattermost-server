...
func testPendingAutoAddTeamMembers(t *testing.T, ss store.Store) {
	...
	require.Len(t, userTeamIDs, 1)
	require.Equal(t, user.Id, userTeamIDs[0].UserID)
	require.Equal(t, team.Id, userTeamIDs[0].TeamID)
...
func testPendingAutoAddChannelMembers(t *testing.T, ss store.Store) {
	...
	require.Len(t, userChannelIDs, 1)
	require.Equal(t, user.Id, userChannelIDs[0].UserID)
	require.Equal(t, channel.Id, userChannelIDs[0].ChannelID)
...
func testPendingTeamMemberRemovals(t *testing.T, ss store.Store) {
	...
	require.Equal(t, data.UserC.Id, idPairs[0].UserID)
	...
	var userIDs []string
	for _, item := range idPairs {
		userIDs = append(userIDs, item.UserID)
	}
	require.Contains(t, userIDs, data.UserB.Id)
	require.Contains(t, userIDs, data.UserC.Id)
	require.Equal(t, data.ConstrainedTeam.Id, idPairs[0].TeamID)
	require.Equal(t, data.ConstrainedTeam.Id, idPairs[1].TeamID)
...
func testPendingChannelMemberRemovals(t *testing.T, ss store.Store) {
	...
	require.Contains(t, userIDs, data.UserB.Id)
	require.Contains(t, userIDs, data.UserC.Id)
	require.Equal(t, data.ConstrainedChannel.Id, idPairs[0].ChannelID)
	require.Equal(t, data.ConstrainedChannel.Id, idPairs[1].ChannelID)
